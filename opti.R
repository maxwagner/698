#libs
require("ROSE") #synthetic
require("pROC") #roc plots
require("rpart") #trees
require("rpart.plot")
require("caret")
...
require("DMwR") #smote
require("RWeka") #c4.5
require("C50") #boosted c5.0
require("ipred") #bagging cart
require("psych")
require("party")
require("partykit")
...
#looking a training data distribution
fraud <- nrow(train_model[train_model$Class == 1,])
notFraud <- nrow(train_model) - fraud
paste("fraud: ", fraud, "|| not fraud: ", notFraud)

#looking a eval data distribution
fraud <- nrow(train_eval[train_eval$Class == 1,])
notFraud <- nrow(train_eval) - fraud
paste("fraud: ", fraud, "|| not fraud: ", notFraud)
...
confusionMatrix(train_eval$Class, round(predict_mlr1))
...
confusionMatrix(train_eval$Class, round(predict_poisson1))
...
confusionMatrix(train_eval$Class, round(predict_logit1))
...
confusionMatrix(train_eval$Class, round(predict_backward))
...
confusionMatrix(train_eval$Class, round(predict_forward))
...
ptm <- proc.time();rforest <- randomForest(Class ~ ., data = train_model, ntree = 250);proc.time() - ptm
predict_rforest <- predict(rforest, train_eval)
confusionMatrix(train_eval$Class, round(predict_rforest))
